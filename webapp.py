...
このアプリは現在、お使いのPC内でのみ動作しています（ローカル環境）。
世界中の誰もがアクセスできるようにするには、ウェブサーバーにこのアプリを配置（デプロイ）する必要があります。

**公開方法の例:**
- **Streamlit Cloud:** Streamlitが公式に提供している無料の共有サービス。GitHubアカウントと連携して、比較的簡単に公開できます。
- **Heroku, AWS, GCPなど:** より本格的なクラウドサービス。設定の自由度が高いですが、専門知識が必要です。

**公開に必要な手順の概要（Streamlit Cloudの場合）:**
1. このプロジェクト（`webapp.py`と`trade_plotter_pinescript.txt`）をGitHubリポジトリにアップロードします。
2. `requirements.txt`というファイルを作成し、`streamlit`, `pandas`, `pytz`といった必要なライブラリ名を記述します。
3. Streamlit Cloudにサインアップし、GitHubリポジトリを連携してアプリをデプロイします。
""")
